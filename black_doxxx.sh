...
# Print "BLACK DOXXX" and credit using figlet with "lean" font style
echo -e "${LIGHT_GREEN}"
figlet -f lean -c "BLACK DOXXX"
figlet -f small -c "made by nikhil rajput"
echo -e "${NC}"
...
# Create a cleaned version of live_subdomains.txt
echo "[*] Creating cleaned version of live subdomains..."
sed -E 's#^https?://##' "$OUTPUT_DIR/live_subdomains.txt" | sed 's#/$##' > "$OUTPUT_DIR/cleaned_live_subdomains.txt"
...
# Run naabu to scan cleaned live subdomains
echo "[*] Running naabu to scan cleaned live subdomains..."
naabu -l "$OUTPUT_DIR/cleaned_live_subdomains.txt" -o "$OUTPUT_DIR/naabu_results.txt"
